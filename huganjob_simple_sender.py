...
# -*- coding: utf-8 -*-
"""
HUGANJOB 簡易プレーンテキスト送信システム
迷惑メール判定対策版 - 配信停止リンクなしの最小構成

作成日: 2025年6月27日
目的: 迷惑メール判定問題の完全解決（配信停止リンクなし版）
特徴: 
- HTMLを完全削除
- 配信停止リンクなし
- 最小限のヘッダー
...
class HuganjobSimpleSender:
    def __init__(self):
        ...
        self.results_file = "huganjob_simple_results.csv"
        self.history_file = "huganjob_simple_history.json"
    ...
    def setup_logging(self):
        ...
        logging.basicConfig(
            ...
            handlers=[
                logging.FileHandler('logs/huganjob_simple_sender.log', encoding='utf-8'),
    ...
    def create_simple_email(self, company_name, job_position, recipient_email):
        """簡易プレーンテキストメールを作成（配信停止リンクなし）"""
        
        # 件名（極めて自然な表現）
        subject = f"{job_position}の採用について"
        ...
        try:
            ...
            for index, row in df.iterrows():
                try:
                    ...
                    # メール作成
                    msg, subject = self.create_simple_email(
